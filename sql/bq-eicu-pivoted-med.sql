...
(
  ...
  -- , loadingdose
  from `physionet-data.eicu_crd_demo`.`medication` m
  -- only non-zero dosages
  where dosage is not null
  -- not cancelled
  and drugordercancelled = false
)
select
patientunitstayid
, drugorderoffset
...
, drugstopoffset
, max(case when drugname_structured = 'norepinephrine' then 1 else 0 end) as norepinephrine
, max(case when drugname_structured = 'epinephrine' then 1 else 0 end) as epinephrine
, max(case when drugname_structured = 'dopamine' then 1 else 0 end) as dopamine
, max(case when drugname_structured = 'dobutamine' then 1 else 0 end) as dobutamine
, max(case when drugname_structured = 'phenylephrine' then 1 else 0 end) as phenylephrine
, max(case when drugname_structured = 'vasopressin' then 1 else 0 end) as vasopressin
, max(case when drugname_structured = 'milrinone' then 1 else 0 end) as milrinone
, max(case when drugname_structured = 'heparin' then 1 else 0 end) as heparin
, max(case when drugname_structured = 'warfarin' then 1 else 0 end) as warfarin
from vw0
WHERE
